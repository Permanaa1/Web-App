...
export default function RecipeGrid({ recipes }) {
  ...
  const allIngredients = useMemo(() => [...new Set(recipes.flatMap(recipe => recipe.ingredients))].map(ingredient => ({ value: ingredient, label: ingredient })), [recipes]);

  const filteredRecipes = selectedIngredients.length > 0
    ? recipes.filter(recipe => selectedIngredients.every(ingredient => recipe.ingredients.includes(ingredient.value)))
    : recipes;

  useEffect(() => {
    cardRefs.current = cardRefs.current.slice(0, filteredRecipes.length);
    ...
    <section>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-8">
        {filteredRecipes.map((recipe, index) => (
          <Link to={`/minuman/${recipe.id}`} key={recipe.id}>
            <div 
